...
- R1: Cuando el plan activo indica emisión para un acumulador en el intervalo vigente, se publica `ON` en el `damper_topic` de ese acumulador para activar su descarga; cuando deja de indicarla, se publica `OFF` para desactivarla.
...
- R3: Mientras la descarga esté activada, se publica la temperatura objetivo vigente en el topic de consigna del acumulador. En un intervalo de anticipación la consigna publicada es la de la próxima consigna del plan que motiva esa emisión.
- R4: Cada ciclo de publicación del proceso MQTT reafirma el estado de descarga y, cuando corresponde, la consigna de cada acumulador. Los mensajes de mando no se publican con retención.
- R5: Se publica `OFF` y no se publica consigna cuando no hay plan activo, el plan es `INVALID`, el control automático está desactivado, el acumulador está en modo `OFF`, hay una prueba de relés en curso o el estado del controlador no está vigente.
...
- R10: Un fallo de publicación MQTT no interrumpe el ciclo de publicación ni el resto de los mandos del mismo ciclo, y se registra de forma colapsada como el resto de fallos de publicación.
...
- A4: Sin plan activo, con plan `INVALID`, con control automático desactivado, con el acumulador en modo `OFF`, durante una prueba de relés y con el estado del controlador no vigente, se publica `OFF`.
...
- A7: Un error del cliente MQTT al publicar el mando de un acumulador deja el ciclo completo, el resto de mandos publicados y un único registro por transición de fallo.
...
- D5b: El mando lo publica el proceso MQTT, no el del controlador, porque el proceso del controlador no tiene cliente MQTT: son contenedores separados y toda la E/S MQTT vive en `backend-mqtt`, que ya lee el plan activo, el estado de control y la prueba de relés de la base de datos. Evita una segunda conexión al broker y que el proceso que acciona los relés dependa de la red MQTT. La cadencia de reafirmación es la de publicación MQTT.
- D5c: La condición «el controlador no está gobernando» se evalúa por la vigencia de su latido, que es la regla de liveness que ya usan el panel y el estado MQTT. Cubre además una caída del controlador, no solo una parada ordenada.
