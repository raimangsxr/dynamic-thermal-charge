...
## Decisions
- D1: `DTC_API_TOKEN` aprovisiona la instalación, pero el token efectivo posterior continúa siendo el digest persistido y puede rotarse desde la administración existente.
- D2: No se añade recuperación de credenciales por correo, token temporal ni puerta trasera; la recuperación requiere acceso administrativo al dispositivo.
- D3: Este cambio se implementará después de `add-critical-flow-e2e-gate` y `establish-shared-ui-language`.

## Tasks
- [ ] T1: Retirar el flujo backend y frontend de onboarding y definir el fallo de inicialización.
- [ ] T2: Validar la credencial antes de crear sesión y coordinar redirecciones por `401`.
- [ ] T3: Actualizar mensajes, documentación y pruebas de inicialización, login, rotación y cierre de sesión.
