...
- [x] T1: Añadir `full_discharge_minutes` y `static_emission_percent` a `Heater` con validación y las propiedades derivadas de potencia de emisión y potencia residual.
- [x] T2: Migración Alembic nueva más `schema.py`, `active_schema.py`, `mapping.py`, `repository.py` y `seed.py` para persistir ambos campos con los valores por defecto de D7.
- [x] T3: Sustituir `_heat_delivery_limit_kwh` por el límite dependiente del estado de carga en `room_energy_step` y en la restricción del MILP, y aplicar la puerta de emisión de R5.
- [x] T4: Exponer el límite aplicado por intervalo en `RoomEnergyInterval` y en el payload de planificación.
- [x] T5: Actualizar los esquemas y rutas de la API con `full_discharge_hours` y `static_emission_percent` y su validación.
- [x] T6: Actualizar el formulario de acumuladores del frontend, sus tipos y su texto de ayuda.
- [x] T7: Cubrir con tests la curva del helper, la restricción del optimizador, la puerta de emisión, el caso `DEGRADED` por capacidad de emisión, el contrato de la API y la migración.
- [x] T8: Actualizar `openspec/specs/planning/spec.md` y la sección del modelo físico de `README.md`; ejecutar `make check`.
