...
const TABLE: Record<ApiErrorCode, Omit<Explained, 'action'> & { action: string }> = {
  ...
  no_configuration: {
    ...
    action: 'Solicita al administrador que inicialice la base de datos en el dispositivo. El panel no puede hacerlo.',
    onDevice: true,
    fieldScoped: false,
  },
  schema_unusable: {
    ...
    action:
      'Solicita al administrador que migre la base de datos en el dispositivo, o actualiza el ' +
      'servicio si el esquema es más nuevo que él. El panel no puede migrar la base de datos.',
    onDevice: true,
    fieldScoped: false,
  },
  store_unavailable: {
