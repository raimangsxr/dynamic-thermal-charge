...
class MqttPublisher:
    ...
    def refresh(self, *, force_discovery: bool = False) -> bool:
        ...
        for command in commands:
            ...
            if payload is not None:
                # The commanded state is distinct from damper position
                # telemetry: the thermostat closes its damper on reaching the
                # setpoint while the discharge stays enabled.
    ...
    def republish_heater(self, heater_id: str) -> None:
        ...
        if payload is not None:
            commands = self._discharge_commands()
            for command in commands:
                if command.heater_id == heater_id:
                    payload["discharge_enabled"] = command.enabled
                    break
            self._publish_json(self._topics.heater_state(heater_id), payload)

    def stop_discharge(self) -> None:
        """Disable every configured discharge before an orderly shutdown."""
        try:
            commands = tuple(self._discharge())
        except Exception:
            logger.error(
                "Could not resolve discharge commands during MQTT shutdown",
                exc_info=True,
            )
            return
        failures: set[str] = set()
        ...
        if failures:
            logger.error(
                "Could not publish the discharge shutdown command of %s",
                sorted(failures),
            )
    ...
    def _publish_command(self, topic: str, payload: str) -> None:
        """Publish commands without retention; the next cycle reasserts them."""
    ...
    def discharge_commands(self) -> tuple[DischargeCommand, ...]:
        ...
        return resolve_discharge_commands(
            ...
            heater_modes=(
                {}
                if control is None
                else dict(getattr(control, "heater_modes", {}) or {})
            ),
...
def _relay_test_in_progress(view: Mapping[str, Any] | None) -> bool:
    ...
    if isinstance(session, Mapping) and session.get("status") in {
        "starting",
        "active",
        "ending",
    }:
