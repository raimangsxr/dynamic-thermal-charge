...
def _upcoming_setpoint(
    ...
    for slot in slots[index + 1 :]:
        target = _float_or_none((slot.get("target_temperature_c") or {}).get(heater_id))
...
def resolve_discharge_commands(
    ...
    *,
    ...
    def command(heater_id: str, enabled: bool, setpoint: float | None) -> DischargeCommand:
    ...
    slots = [item for item in (plan.get("slots") or ()) if isinstance(item, Mapping)]
